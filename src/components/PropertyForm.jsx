...
import React, { useState, useEffect } from 'react';
import { getProperties, addProperty, updateProperty, deleteProperty } from '../api/propertyService'; // Import updated propertyService
import '../styles.css'; // Import specific styles for PropertyManagement
...
const PropertyForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedProperties = await getProperties(); // Refresh property list
      setProperties(updatedProperties);
      resetForm();
    } catch (error) {
      setError('Error saving property!');
