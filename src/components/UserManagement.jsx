...
const UserManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        await userService.updateUser(editingUser.id, newUser); // Update existing user
        alert('User updated successfully!');
      } else {
        await userService.createUser(newUser); // Register new user
        alert('User added successfully!');
      }
      const updatedUsers = await userService.getUsers(); // Refresh user list
